...
class StudentDetailView(View):
    def get(self, request, pk):
        ...
        return JsonResponse({
            ...
            "avg_behaviour": float(student.avg_behaviour) if student.avg_behaviour is not None else None,
            "avg_marks": float(student.avg_marks) if student.avg_marks is not None else None
...
class WritingDatasetListView(View):
    def get(self, request):
        data = list(FactWritingDataset.objects.values())
        return JsonResponse(data, safe=False, json_dumps_params={'ensure_ascii': False})


class ShapesDatasetListView(View):
    def get(self, request):
        data = list(FactShapesDataset.objects.values())
        return JsonResponse(data, safe=False, json_dumps_params={'ensure_ascii': False})


class EmotionsDatasetListView(View):
    def get(self, request):
        data = list(FactEmotionsDataset.objects.values())
        return JsonResponse(data, safe=False, json_dumps_params={'ensure_ascii': False})


class AutismSurveyListView(View):
    def get(self, request):
        data = list(FactAutismTeacherSurveyDataset.objects.values())
        return JsonResponse(data, safe=False, json_dumps_params={'ensure_ascii': False})
    
class TeacherSurveyListView(View):
    def get(self, request):
        data = list(FactTeacherSurveyDataset.objects.values())
        return JsonResponse(data, safe=False, json_dumps_params={'ensure_ascii': False})
